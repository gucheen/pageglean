...
`PAGEGLEAN_WEBHOOK_TOKEN` 可选，配置后每次通知（包括重试和手动发送）都会附带 `X-Webhook-Token` 请求头，值为配置的令牌原文。留空时不发送此请求头。令牌仅接受可见 ASCII 字符，不含空格或换行；它用于接收端认证，与 HMAC 签名密钥独立，不替代现有签名。令牌不会通过状态接口或错误信息返回。
