...
func (a *App) handleExport(w http.ResponseWriter, r *http.Request) {
	...
	switch format {
	case "", "json":
		w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="links-%s.json"`, date))
		writeJSON(w, http.StatusOK, map[string]any{
			"version": 1, "exportedAt": time.Now().UTC(), "bookmarks": bookmarks,
		})
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="links-%s.csv"`, date))
	...
	case "html":
		...
		w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="links-%s.html"`, date))
		...
		_, _ = fmt.Fprintln(w, `<TITLE>Links Export</TITLE><H1>Links Export</H1><DL><p>`)
		for _, bookmark := range bookmarks {
